...
public sealed class JsonLogRepository : ILogRepository
{
    ...
    public JsonLogRepository(LogStore store) => _store = store;
    public async Task SaveAsync(LogBatch batch) { var items = await _store.ReadAllAsync(); items.Add(batch); await _store.WriteAllAsync(items); }
}
